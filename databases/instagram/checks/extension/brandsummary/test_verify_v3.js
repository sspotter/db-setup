...
async function verifyMetricsV2(brand) {
    try {
        const dataPath = path.join(__dirname, 'igtest3.json');
        ...
        const totalCollabReach = collaboratorEntries.reduce((sum, r) => sum + r.potential_reach, 0);
        
        reachBreakdown += `Reach Breakdown (Collaborators) (TOTAL INFULENCERS IMPRESSIONS ${formatNumber(totalCollabReach)}):\n`;
        collaboratorEntries.forEach(r => {
            const displayReach = r.follower_count > 0 ? formatNumber(r.potential_reach) : 'N/A';
            reachBreakdown += ` - @${r.username} (${r.role}): ${displayReach} (${r.posts_involved} posts)\n`;
        });
        ...
        // Write to file
        fs.writeFileSync('outputdata/test_output_cleanV2.txt', outputData, 'utf8');
