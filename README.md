...
## Prisma Studio (browse the tables)

A Prisma schema is provided for **each** database so you can browse its tables in
Prisma Studio. The model definitions are generated by introspection
(`prisma db pull`) and live in `prisma/tiksurfer.prisma` and `prisma/insta.prisma`.

```bash
# (re)introspect after the schema changes — refreshes the Prisma models
npm run introspect            # both, or: introspect:tiksurfer / introspect:insta

# open Studio (each DB on its own port; leave running, Ctrl-C to stop)
npm run studio:tiksurfer      # Tik_Surfer_multi_fix    -> http://localhost:5555
npm run studio:insta          # insta_surf_multi_prisma -> http://localhost:5556
```

Run them in separate terminals to view both databases at once. Studio reads the
connection string from `.env` (`TIKSURFER_DATABASE_URL` / `INSTA_DATABASE_URL`),
the same source of truth used to create the databases.
